...
import com.zqw.service.EmployeeService;
...
public class EmployeeController {
    ...
    public Result selectList(Employee employee){     // get请求能够将query参数转化为对象中的属性
    ...
    public Result pageQuery(
                            Employee employee,      // 可将query参数中的归属于employee的属性赋值给该对象
                            @RequestParam(defaultValue = "1") Integer pageNum,
                            @RequestParam(defaultValue = "10") Integer pageSize){
        PageInfo<Employee> page = employeeService.pageQuery(employee, pageNum, pageSize);
        return Result.success(page);
    }
    ...
    public Result deleteBatch(@RequestBody List<Integer> list){
        employeeService.removeBatch(list);
        return Result.success();
    }
}
